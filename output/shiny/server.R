server <- function(input, output, clientData, session) {
             ...
             lapply(s, function(i) {
               a(box(width=NULL, height = 200,
                     title = HTML(paste0("<div class='image-wrap'><img src='./images/poster/",
                                         ...
                                         "</strong><br>"))
               ), href= as.character(name.link$link[name.link$name == i]), target="_blank")
             })
      )
    )
  ...
  output$wc <- renderUI(
    div(img(src=paste0('./images/word_cloud/wc', input$char1, '.png'), 
            width = "55%", height = "450px"), style="text-align: center;")
  )
  
  output$table3 <- renderTable({
    matrix(as.character(as.matrix(da[input$char1,2:6])), dimnames = list(rep("", 5), c("Names")))
  })
